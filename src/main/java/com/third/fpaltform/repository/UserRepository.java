...
public interface UserRepository extends CrudRepository<UserEntity, Integer> {
	 ...
	 * @return
	 */
	@Query("select u from UserEntity u where u.uid <> :userId and u.account = :userName and u.status !=0 ")
	...
	@Query( value = "select u from UserEntity u where u.division.uid = ?1 and u.status !=0 and u.account like concat('%', ?2, '%')")
	...
	@Query( value = "select u from UserEntity u where u.status !=0 and u.account like concat('%', ?1, '%')")
